...
angular.module('tm.pagination', []).directive('tmPagination',[function(){
    return {
        restrict: 'EA',
        template: '<div class="page-list ng-isolate-scope" conf="paginationConf">' +
        '<div class="page-total" ng-show="conf.totalItems > 0">每页' +
        '<select select ng-model="conf.itemsPerPage" ng-options="option for option in conf.perPageOptions " ng-change="changeItemsPerPage()">'+
        '</select>' +
        ...
        'ng-click="changeCurrentPage(item)"><span class="ng-binding">{{ item }}</span></li> ' +
        '<li ng-class="{disabled: conf.currentPage == conf.numberOfPages}" ng-click="nextPage()"><span class="ng-binding">&raquo;</span></li> ' +
        '</ul>'+
        '<div class="no-items ng-hide" ng-show="conf.totalItems <= 0">暂无数据</div>'+
        '</div>',
        replace: true,
        scope: {
            conf: '='
