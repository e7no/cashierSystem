...
$url = $config_host . '/service/sto/viceScreen/getById/' . $id;
$json = http($url, '', 1);
$list = $json['datas']['viceScreenImgVO'];
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <div class="content">
        <div class="wbox">
            <div class="wbox-content add-vice">
                <form class="layui-form layui-form-pane" id='myupload' action='../../Controller/cashier/upload.php' method='post' enctype='multipart/form-data'>
                            ...
                            <div class="layui-input-inline" style="width: 278px">
                                ...
                                <a href="javascript:;" class="choose-book">
                                    <input type="file" id="upload" multiple="" accept="image/jpeg,image/jpg,image/png"
                                           onchange="angular.element(this).scope().img_upload(this.files)"/>浏览
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="layui-form-item" style="text-align: center;"><span id="uploading"><img style="width:330px;height:auto;" ng-src="{{img}}"></span>
                    </div>
...
<script type="text/javascript">
    ...
    app.controller('listController', function ($scope, $http, Util) {
        $scope.title = '<?php echo $list['name'];?>';
        $scope.sn = '<?php echo $list['sn'];?>';
        $scope.status = '<?php echo $list['status'];?>';
        $scope.img = '<?php echo $list['imgPath'];?>';
        $('#picture').val('<?php echo $list['imgPath'];?>');
        if ($scope.status == '1') {
            $scope.isActive = true;
        } else {
