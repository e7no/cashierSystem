                ...
                <div class="con-table">
                    <table class="layui-table" style="min-width: 1000px;">
                        ...
                        <tr class="text-c">
                            <th width="10%">日期</th>
                            <th width="10%">门店名称</th>
                            <th width="11%">门前人流量</th>
                            <th width="10%">进店客流量</th>
                            <th width="9%">进店转化率</th>
                            <th width="9%">进店跳出量</th>
                            <th width="9%">进店跳出率</th>
                            <th width="9%">新客量</th>
                        ...
                        <tr class="text-c" ng-repeat="item in list">
                            ...
                            <td>{{item.outside}}</td>
                            <td>{{item.instore}}</td>
                            <td>{{item.inConvRate*100}}%</td>
                            <td>{{item.bounce}}</td>
                            <td>{{item.outConvRate*100}}%</td>
                            <td>{{item.freshCustomer}}</td>
                            <td>{{item.oldCustomer}}</td>
                        </tr>
                        </tbody>
                    </table>
                </div>
...
<script type="text/javascript">
    ...
    app.controller('listController', function ($scope, $http) {
        ...
        $('#excel').click(function () {
            ...
            window.location.href = '../../Controller/Excel/dataAnalysisTabelExcel.php?stoName=' + stoName + '&indexDate=' + indexDate;
        });
    });
</script>
</body>
