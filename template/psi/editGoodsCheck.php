                ...
                <form class="layui-form layui-form-pane psi-form">
                    <div class="con-table">
                        <table class="layui-table" style="min-width: 1600px;">
                            ...
                            <tr class="text-c" ng-repeat="item in list">
                                ...
                                <td><input type="text" class="egs-text" placeholder="盘点数量" ng-model="$parent.num[$index]"/></td>
                                <td>{{item.salePrice*$parent.num[$index]?(item.salePrice*$parent.num[$index]|number:2):'0.00'}}</td>
                                <td>{{$parent.num[$index]?($parent.num[$index]-item.storeNum):0}}</td>
                                <td>{{$parent.num[$index]?(($parent.num[$index]-item.storeNum)*item.salePrice):0}}</td>
                            </tr>
                            <tr class="text-c">
                                <td><input type="checkbox"/></td>
                                <td colspan="2" class="text-l psi-select" ng-click="addGoodModal()">
                                    请选择盘点商品...
                                </td>
                                <td></td>
                                <td></td>
                    ...
                    <div class="layui-form-item">
                        ...
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline" style="width: 120px;">
                                <input type="text" class="layui-input" ng-model="name" placeholder="请输入盘点员姓名">
                            </div>
                        </div>
                    </div>
                    <div class="layui-form-item" style="margin-top: 10px;">
                        ...
                        <div class="layui-input-inline" style="width: 380px;">
                            <input type="text" class="layui-input" ng-model="note" placeholder="请输入商品盘点备注">
                        </div>
                    </div>
                    <div class="layui-form-item" style="margin-top: 10px;">
                        <input type="button" class="layui-btn layui-btn-small layui-btn-normal put-btn" value="盘点" disabled="disabled"
                               ng-show="!readDrivingUnable" ng-click="deleteModal()">
                        <input type="button" class="layui-btn layui-btn-small layui-btn-normal put-btn" value="盘点" ng-show="readDrivingUnable"
                               ng-click="deleteModal()">
...
<script type="text/javascript">
    ...
    app.controller('listController', function ($scope, $http) {
        ...
        $scope.deleteModal = function () {
            ...
            if (num == 0) {
                layer.msg('请添加盘点商品再进行盘点！', {time: 3000});
                $scope.readDrivingUnable = true;
                return;
            }
                ...
                if ($scope.list[i].skuId == '') {
                    ...
                    if ($scope.list[i].goodsId != '') {
                    	if ($scope.num[i] == '' || angular.isUndefined($scope.num[i])) {
                            layer.msg('请填写盘点数量！', {time: 3000});
                            $scope.readDrivingUnable = true;
                            return;
                        }else if (Number($scope.num[i]) == 0) {
                            layer.msg('填写盘点数量必须大于 0！', {time: 3000});
                            $scope.readDrivingUnable = true;
                            return;
                        } else {
                            detailList[i]['invQty'] = $scope.num[i];
                        }
                    }
                } else {
                    detailList[i]['goodsId'] = $scope.list[i].goodsId;
            ...
            $http.post('../../Controller/psi/listGoodsCheckAction.php', postData).then(function (result) {  //正确请求成功时处理
                if (result.data.success) {
                    ...
                    parent.layer.msg('恭喜你，盘点成功！', {icon: 6, time: 1500});
