...
<!--                    </div>-->
                    <div class="layui-inline">
                        ...
                        <div class="layui-input-inline" style="width: 150px;">
                            <input type="text" class="layui-input" placeholder="请输入盘点员姓名" ng-model="operator">
                        </div>
                    </div>
    ...
    app.controller('listController', function ($scope, $http) {
        var reSearch = function () {
            var postData = {
                type: 'list',
                no: $scope.no,
                invDateStart: $scope.createDateStart,
                invDateEnd: $scope.createDateEnd,
        ...
        $scope.checkDetail = function (id) {
            layer.open({
                ...
                shadeClose: true,
                offset: ['0', '0'],
                content: 'detailsGoodsCheck.php?id=' + id,
            });
        }
        $('#excel').click(function () {
            var no = $scope.no ? $scope.no : '';
            var invDateStart = $scope.createDateStart ? $scope.createDateStart : '';
            var invDateEnd = $scope.createDateEnd ? $scope.createDateEnd : '';
            var operator = $scope.operator ? $scope.operator : '';
            layer.msg('数据导出中，请耐心等待....！', {time: 2500});
            window.location.href = '../../Controller/Excel/listGoodsCheckExcel.php?no=' + no + '&invDateStart=' + invDateStart + '&invDateEnd=' + invDateEnd + '&operator=' + operator;
        });
    });
</script>
</body>
