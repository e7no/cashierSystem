					...
					<div class="layui-form-item">
						...
						<div class="layui-inline">
					    	...
					    	<div class="layui-input-inline" style="width: 110px;">
					    		<input type="text" class="Wdate layui-input" ng-model="inDateStart" onchange="" id="logmin" placeholder="请选择..."  onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'logmax\')||\'%y-%M-%d\'}'})">
					    	</div>
					    	<div class="layui-form-mid">-</div>
					    	<div class="layui-input-inline" style="width: 110px;">
					    		<input type="text" class="Wdate layui-input" ng-model="inDateEnd" onchange="" id="logmax" placeholder="请选择..." onfocus="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d'})">
					    	</div>
					    </div>
<!-- 					    <div class="layui-inline"> -->
    ...
    app.controller('listController', function ($scope, $http) {
        ...
        var reSearch = function () {
            ...
            $http.post('../../Controller/psi/listGoodsStorageAction.php', postData).then(function (result) {  //正确请求成功时处理
                ...
                angular.forEach($scope.Detail, function (item) {
                	$scope.inStock = $scope.inStock+$scope.Detail[i].inStock;
                    $scope.amount = $scope.amount+($scope.Detail[i].inPrice*$scope.Detail[i].inStock);
                    i++;
                })
        ...
        $scope.detail=function(id){
        	layer.open({
    			...
    			shadeClose: true,
    			offset: ['0', '0'],
    			content: 'detailsGoodsStorage.php?id='+id,
    		});
        }
