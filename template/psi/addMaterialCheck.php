...
<body ng-app="myApp">
						...
						<table class="layui-table" style="min-width: 600px;">
							...
							<tbody>
								<tr class="text-c" ng-repeat="itemsg in goodslist | filter:{'typeId':active}:true" ng-cloak>
									<td><input type="checkbox"  ng-model="$parent.confchil[itemsg.id]" ng-change="selectOne(itemsg.typeId,itemsg.id,itemsg.code,itemsg.name,itemsg.spec,itemsg.unit,itemsg.stock,itemsg.standardPrice,itemsg.typeName,itemsg.avgPrice)" ng-change="selectOne()"></td>
									<td>{{itemsg.code}}</td>
									<td class="text-l">{{itemsg.name}}</td>
									<td>{{itemsg.spec}}</td>
									<td>{{itemsg.unit}}</td>
									<td>{{itemsg.avgPrice}}</td>
									...
									<input type="checkbox"  ng-model="$parent.confchil9[itemsg.avgPrice]" style="display: none;" />
								</tr>
							</tbody>
						</table>
	...
	<form action="editMaterialCheck.php" id="sub" method="post" target="_parent">
...
app.controller('listController', function ($scope, $http) {
    ...
    $http.post('../../Controller/psi/addMaterialCheckAction.php', postData).then(function (result) {  //正确请求成功时处理
        ...
        angular.forEach($scope.goodslist, function (item) {
	        angular.forEach($scope.list, function (items) {
	        	if (item.id == items.id) {
	                ...
	                $scope.confchil9[item.avgPrice] = true;
	                $scope.checked9.push(item.avgPrice);
	            }
	        }) 
        ...
        if ($scope.conf[id]) {
            ...
            angular.forEach($scope.goodslist, function (item) {
                if (item.typeId == id) {
                    ...
                    $scope.confchil9[item.avgPrice] = false;
                    $scope.checked9.splice($.inArray(item.avgPrice, $scope.checked9), 1);
                }
            })
        }
    }
    $scope.selectOne = function (partentId, id, code, name, spec, unit, stock, standardPrice,typeName,avgPrice) {
    	$scope.partentIdarr = [];
        $scope.arr = [];
        ...
        var index9 = $scope.checked9.indexOf(avgPrice);
    ...
    $scope.AddMaterial = function(){
        ...
        for (var i = 0; i < count; i++) {
            ...
            datas[i]['standardPrice'] = $scope.checked6[i] ? $scope.checked6[i] : 0;
            datas[i]['typeName'] = $scope.checked7[i];
            datas[i]['typeId'] = $scope.checked8[i];
            datas[i]['avgPrice'] = $scope.checked9[i] ? $scope.checked9[i] : 0;
        } 
