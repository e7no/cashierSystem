...
if ($type == "list") {
    ...
    $url = $config_host . '/service/gds/material/deleteMaterial';
    ...
    echo json_encode($str);
    exit;
} else if ($type == 'delMore') {
    $ids = object2array($postData->delIds);
    $delIds = implode(',', $ids);
    $url = $config_host . '/service/gds/material/deleteMaterial';
    ...
    $dealId = $postData->id;
    $url = $config_host . '/service/gds/material/changeOpenState';
    $datas = array('datas' => array('ids' => $dealId));
    $json = http($url, $datas, 1);
    if ($json['success']) {
        $str['success'] = true;
        if ($json['errMsg'] == '') {
