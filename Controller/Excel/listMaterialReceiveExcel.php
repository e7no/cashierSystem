...
$no = $_GET['no'];
$outDateStart = $_GET['createDateStart'];
$outDateEnd = $_GET['createDateEnd'];
...
if ($outDateStart != '') {
    $datas['datas']['outDateStart'] = $outDateStart;
}
if ($outDateEnd != '') {
    $datas['datas']['outDateEnd'] = $outDateEnd;
}
if ($personName != '') {
    $datas['datas']['personName'] = $personName;
}
...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('B1', '领用总量')
    ->setCellValue('C1', '领用总额')
    ->setCellValue('D1', '领用时间')
    ->setCellValue('E1', '备注')
    ->setCellValue('F1', '领用员')
    ->setCellValue('G1', '操作员');
...
$objPHPExcel->getActiveSheet()->setTitle('原料领用单列表');
...
header('Content-Disposition: attachment;filename="原料领用单列表.xls"');//导出文件
