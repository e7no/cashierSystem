...
$deltailId = $_GET['id'];
$url = $config_host . '/service/gds/material/scrap/detail/' . $deltailId;
...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('G1', '原料单价')
    ->setCellValue('H1', '报废数量')
    ->setCellValue('I1', '报废金额')
    ->setCellValue('J1', '当前库存数量');
...
foreach ($list as $key => $value) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $key, ' ' . $value['scrapPrice']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $key, ' ' . $value['scrapStock']);
...
$objPHPExcel->getActiveSheet()->setTitle('报废原料详情');
...
header('Content-Disposition: attachment;filename="报废原料详情.xls"');//导出文件
