...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('E1', '规格')
    ->setCellValue('F1', '库存')
    ->setCellValue('G1', '采购标准价')
    ->setCellValue('H1', '采购最高价')
    ->setCellValue('I1', '状态')
...
foreach ($list as $key => $value) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('F' . $key, ' ' . $value['stock']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $key, ' ' . $value['standardPrice']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $key, ' ' . $value['maxPrice']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('I' . $key, $value['openState'] ? '启用' : '关闭');
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('J' . $key, $value['modifyDate']);
...
$objPHPExcel->getActiveSheet()->setTitle('原料列表');
...
header('Content-Disposition: attachment;filename="原料列表.xls"');//导出文件
