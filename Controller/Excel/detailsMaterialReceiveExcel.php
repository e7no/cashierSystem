...
$objPHPExcel->setActiveSheetIndex(0)
    ...
    ->setCellValue('D1', '单位')
    ->setCellValue('E1', '规格')
    ->setCellValue('F1', '领用标准价')
    ->setCellValue('G1', '原料单价')
    ->setCellValue('H1', '领用数量')
    ->setCellValue('I1', '领用金额')
...
foreach ($list as $key => $value) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('G' . $key, ' ' . $value['outPrice']);
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue('H' . $key, ' ' . $value['outStock']);
...
$objPHPExcel->getActiveSheet()->setTitle('领用原料详情');
...
header('Content-Disposition: attachment;filename="领用原料详情.xls"');//导出文件
