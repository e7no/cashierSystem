...
if ($type == "list") {
    ...
    $no = $postData->no;
    $invDateStart = $postData->invDateStart;
    $invDateEnd = $postData->invDateEnd;
    ...
    if ($invDateStart != '') {
        $datas['datas']['invDateStart'] = $invDateStart . ' 00:00:00';
    }
    if ($invDateEnd != '') {
        $datas['datas']['invDateEnd'] = $invDateEnd . ' 23:59:59';
    }
    ...
    $json = http($url, $datas, 1);
    $str['list'] = $json['datas']['list'];
    $str['inventoryStockInfo'] = $json['datas']['inventoryStockInfo'];
    $str['total'] = $json['datas']['page']['totalSize'];
    echo json_encode($str);
    exit;
} else if ($type == 'cat') {
    ...
    $url = $config_host . '/service/gds/stock/inventory/save';
    $json = http($url, $datas, 1);
    if ($json['success']) {
        unset ($_SESSION['checked']);
        unset ($_SESSION['skuId']);
