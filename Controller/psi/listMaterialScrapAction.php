...
if ($type == "list") {
    ...
    $url = $config_host . '/service/gds/material/scrap/list';
    $datas = array('datas' => array(
        'pageNo' => $currentPage,
        'pageSize' => $perPage,
        'stoId' => $stoId,
        'scrapDateStart' => $scrapDateStart,
        'scrapDateEnd' => $scrapDateEnd,
        'no' => $no,
        'personName' => $operator
    ));
    ...
    $json = http($url, $datas, 1);
    $str['list'] = $json['datas']['list'];
    $str['outStockInfo'] = $json['datas']['vo'];
    $str['total'] = $json['datas']['page']['totalSize'];
    echo json_encode($str);
    exit;
} else if ($type == 'cat') {
    ...
    $url = $config_host . '/service/gds/material/scrap/add';
    $json = http($url, $datas, 1);
    if ($json['success']) {
        unset ($_SESSION['cats']);
        $str['success'] = true;
